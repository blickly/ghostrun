...
signal are shown in Figure~\ref{fig:latencyVsRSSI} and Figure~\ref{fig:latencyVsEcIo}, respectively. We were surprised to find that there was no correlation between signal strength ans latency to query the server. While it is clear that a phone with a sufficiently weak signal will be unable to query the server at all, we found that for signal strengths within the normal operating ranges that we were able to vary, signal strength had no effect on latency. For other applications, it may be interesting to compare signal strength with data bandwidth, but we did not try to make any such comparison.
