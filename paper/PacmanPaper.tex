...
App Engine, which takes care to coordinate and mediate the actions of each of
...
so that the maps can be persistently backed up, and also so that players
...
\caption{A server on Google App Engine is used to coordinate communication between the phone clients.}
...
App Engine service, and the clients are smartphones running the Android 2.2
...
solution. In this approach, the application must register for the permission to
read incoming SMS messages, and then relies on the server to send messages from
a specific number with a given format. This approach has the disadvantage that
it requires unconventional permissions that may make users suspicious, and it
is difficult and expensive to deploy widely, especially when dealing with
...
We used a Python web server running on top of Google's App Engine
service. Google's App Engine service provides many advantage for our
Android application. The service has a built-in high replication datastore,
...
and use those maps. Here, we make use of the App Engine
...
The clients are implemented as a native Android application that runs on any
...
The Game Mapview is the primary interface of the application during the most
import mode -- the gameplay.
...
The client's game loop for a ghost is even simpler and can be described as:
...
the round-trip time of two separate GPS measurements as seen by a
...
and a dialog to close the application appears. Because the drawing is done in a
...
by unauthorized clients or other abuse of the game API.  For example, a
...
Player could even potentially create their own Android application or modify
the existing one to make it send out fake GPS locations.  More work to make the
...
that the other players are performing ``teleportation'' due to their ability to
...
ghost or a pacman) includes other useful features (click listeners, dragging
...
Another App Engine feature that we would like to have used but did not have time
...
the datastore on App Engine, so a game deleting task queue could act as a
...
power pallets on the map. The pacman characters will enter an invincible mode
...
the Android application, the App Engine server, the phone simulator for stress
testing, and even this paper are publicly available, and can be accessed from
